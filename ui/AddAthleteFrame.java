...
public class AddAthleteFrame extends ChildWindow implements ActionListener {  //κανει extend το childwindow ωστε να μπορει να επιστρεφει στο προηγουμενο παραθυρο
	...
	int day;      //χρησιμοποειται για την μετατροπη string σε int ωστε να γινουν πραξεις με την ημερομηνια 
	int month;       //χρησιμοποειται για την μετατροπη string σε int ωστε να γινουν πραξεις με την ημερομηνια 
	int year;         //χρησιμοποειται για την μετατροπη string σε int ωστε να γινουν πραξεις με την ημερομηνια 
	Long phone;        //χρησιμοποειται για την μετατροπη string σε long ωστε να γινουν πραξεις με το τηλεφωνο
	...
	JLabel genderLabel,proffesionalLabel,skillLabel,dateLabel,messageCongratsLabel,messageWrongDateLabel,messageNoDateLabel,   
	messageNoPhoneLabel,messageWrongPhoneLabel,messageNoGenderLabel,messageNoProffesionalLabel,messageNoSkillLabel;
	Border border;

	AddAthleteFrame(JFrame parentWindow, AthleteService athleteService) {           //το parent window το καλει ωστε να μπορει να επιστρεψει πισω
		...
		messageCongratsLabel=new JLabel("Η προσθήκη του αθλητή ήτανε επιτυχής!");              //ολα αυτα ειναι label για αν εχει δωθει λαθος πληροφορια
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==button ) {                    
			if(maleRadioButton.isSelected()) {   //αν πατηθει το κουμπι της αποθηκευσης φτιαχνει της μεταβλητες που χρειαζονται για να εξετασουμε αν ειναι σωστες οι πληροφοριες
				athleteGender='M';
			}
		...
		if(e.getSource()==button ) {                             //στην συνεχεια κανει ολα αυτα τα label αορατα γτ αλλιως αν εχει εμφανιστει αλλο θεμα πριν δεν θα φυγει το μυνημα απο πριν
			messageCongratsLabel.setVisible(false);
			messageNoDateLabel.setVisible(false);
			messageWrongDateLabel.setVisible(false);
			messageNoPhoneLabel.setVisible(false);
			...
			if(textField61.getText().equals("DD") || textField62.getText().equals("MM") || textField63.getText().equals("YYYY")) {     //αν δεν εχει δωθει ημερομηνια
				messageNoDateLabel.setVisible(true);                //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
				...
				if(athleteService.ckeckAthleteDate(day,month,year)){              //και τσεκαρει αν ειναι επιτρεπτες
					messageWrongDateLabel.setVisible(true);                 //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
					wrongInformation=true;	
				}
			}	
			if(textField7.getText().equals("Κινητό τηλέφωνο")) {          //αν εχει δωθει τηλεωονο
				messageNoPhoneLabel.setVisible(true);                      //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
				wrongInformation=true;	
			}
			else {
				phone=Long.parseLong(textField7.getText());
				if(athleteService.checkAthletePhone(phone)) {     //αν το τηλεφονο ειναι 10 ψηφιων 
					messageWrongPhoneLabel.setVisible(true);           //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
					wrongInformation=true;
				}
			}
			if(gender.getSelection()==null) {                //αν στο group δεν εχει επιλεχθει κατι
				messageNoGenderLabel.setVisible(true);   //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
				wrongInformation=true;
			}
			if(proffesional.getSelection()==null) {                //αν στο group δεν εχει επιλεχθει κατι
				messageNoProffesionalLabel.setVisible(true);       //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
				wrongInformation=true;
			}
			if(skill.getSelection()==null) {                //αν στο group δεν εχει επιλεχθει κατι
				messageNoSkillLabel.setVisible(true);        //το αναλογο label γινεται ορατο και ενημερωνει το συστημα οτι υπαρχει λαθος πληροφορια
				wrongInformation=true;
			}
			...
			athleteService.addAthlete(textField3.getText(),textField4.getText(),athleteGender,             //προσθετη αθλητη
					textField61.getText()+"/"+textField62.getText()+"/"+textField63.getText(),textField7.getText(),
